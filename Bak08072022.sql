...
CREATE TABLE `cat_libros` (
  ...
  `detalles_libros` text COLLATE utf8_spanish_ci DEFAULT NULL,
...
(12,27,2,1,1,'ddd','2022-07-08 12:07:05','2022-07-08 12:07:15',1);
...
insert  into `usuarios`(`id_usuaarios`,`nombre`,`primer_apellido`,`segundo_apellido`,`fec_nac`,`correo_elec`,`contrasenia`,`id_perfil`,`visible`,`usuario`) values 
(1,' sandra daniela ','martinez ','ibarra','2003-09-23','ibarrasdaniela@gmail4','b1226879bfdb68676e460f03e8f85a96',1,1,NULL),
(2,'Erick Yahir','Sandoval','Rodriguez','2003-09-26',NULL,NULL,3,1,NULL),
(3,'daniela','martinez','ibarra',NULL,'ibarrasdaniela','daniela',1,1,NULL),
(4,'ee',NULL,NULL,NULL,NULL,NULL,NULL,1,NULL),
(8,'Paola','Romero','Reinoso',NULL,'ibarrasdaniela@gmail','12',3,1,'19211040130091');
...
/*!50001 CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vw_listado_prestamo` AS (select `a`.`id_prestamos` AS `id_prestamos`,concat(`c`.`nombre`,' ',`c`.`primer_apellido`,' ',`c`.`segundo_apellido`) AS `solicita`,concat(`d`.`nombre`,' ',`d`.`primer_apellido`,' ',`d`.`segundo_apellido`) AS `entrega`,`b`.`descrip_licenciaturas` AS `descrip_licenciaturas`,`b`.`descrip_asinaturas` AS `descrip_asinaturas`,`b`.`descrip_libros` AS `descrip_libros`,date_format(`a`.`fecha_prestamo`,'%d-%m-%Y %H:%m%p') AS `fecha_prestamo`,`a`.`retorno_libro` AS `retorno_libro`,`a`.`fecha_retorno` AS `fecha_retorno` from (((`prestamos_libros` `a` left join `vw_libros` `b` on(`a`.`id_libro` = `b`.`id_libros_carrera_asignaturas`)) left join `usuarios` `c` on(`a`.`nombre_recibe` = `c`.`id_usuaarios`)) left join `usuarios` `d` on(`a`.`nombre_prestador` = `d`.`id_usuaarios`))) */;
